...
def _require_cfg() -> dict:
    ...
    if missing:
        st.error("Missing MS Graph config: " + ", ".join(missing))
        st.stop()
    return cfg
...
def login_ui(scopes=None):
    """
    Render ONE sign-in button and handle callback.
    This function should ONLY be called in app.py.
    """
    app = _msal_app()
    cfg = _require_cfg()
